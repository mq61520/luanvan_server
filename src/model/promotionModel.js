...
Promotion.get_promotion_by_id = (km_id, result) => {
  dbConn.query(
    `select km_id, km_value, DATE_FORMAT( CONVERT_TZ(km_ngaybatdau, '+00:00', '+07:00'), '%d/%m/%Y') as ngaybatdau, DATE_FORMAT( CONVERT_TZ(km_ngayketthuc, '+00:00', '+07:00'), '%d/%m/%Y') as ngayketthuc from khuyen_mai where km_id = '${km_id}'`,
    (err, q_result) => {
      if (err) {
        console.log(err);
      } else {
